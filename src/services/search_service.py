...
def filter_outliers(coords_list: List[Tuple[float, float]], max_degrees: float = settings.country_center_outlier_degrees) -> List[Tuple[float, float]]:
    """
...
class SearchService:
    ...
    @staticmethod
    async def determine_search_mode(query: str, lang: str = 'en') -> str:
        """
        ...
        try:
            ...
            countries_count = await SearchService.get_countries_count_by_prefix(query, lang)
            if countries_count > 0:
                has_any_results = True

            # Check cities with prefix
            if not has_any_results:
                cities_count = await SearchService.get_cities_count_by_prefix(query, lang)
                if cities_count > 0:
                    has_any_results = True

            # Check airports with prefix
            if not has_any_results:
                airports_count = await SearchService.get_airports_count_by_prefix(query, lang)
                if airports_count > 0:
                    has_any_results = True
        ...
        try:
            ...
            if not search_query:
                ...
                data = await SearchService.get_phase1_countries(
                    search_query, offset, limit, mode, lang
                )
                # Check if there are more countries
                total_countries = await SearchService.get_countries_count_by_mode(search_query, mode, lang)
            ...
            phase1_count = await SearchService.get_countries_count_by_mode(search_query, mode, lang)
            phase2_count = await SearchService.get_countries_with_cities_count_by_mode(search_query, mode, lang)
            phase3_count = await SearchService.get_countries_with_airports_count_by_mode(search_query, mode, lang)
            ...
            if phase1_count > 0 and offset < phase1_count:
                ...
                phase_offset = offset
                data = await SearchService.get_phase1_countries(
                    search_query, phase_offset, limit, mode, lang
                )
    ...
    @staticmethod
    async def has_phase2_results(query: str, mode: str, lang: str = 'en') -> bool:
        """Check if there are any phase 2 results (countries with matching cities)"""
        count = await SearchService.get_countries_with_cities_count_by_mode(query, mode, lang)
        return count > 0

    @staticmethod
    async def has_phase3_results(query: str, mode: str, lang: str = 'en') -> bool:
        """Check if there are any phase 3 results (full hierarchy)"""
        count = await SearchService.get_countries_with_airports_count_by_mode(query, mode, lang)
        return count > 0

    # ================ COUNTING METHODS ================

    @staticmethod
    async def get_countries_count_by_mode(query: str, mode: str, lang: str = 'en') -> int:
        ...
        try:
            ...
            async with db.get_connection() as conn:
                ...
                sql = f"""
                    SELECT COUNT(*)
                    FROM countries
                    WHERE LOWER(COALESCE(name_translations->>'{lang}', name)) LIKE LOWER($1)
                """
    ...
    @staticmethod
    async def get_countries_with_cities_count_by_mode(query: str, mode: str, lang: str = 'en') -> int:
        ...
        try:
            ...
            async with db.get_connection() as conn:
                ...
                sql = f"""
                    ...
                    WHERE LOWER(COALESCE(city.name_translations->>'{lang}', city.name)) LIKE LOWER($1)
                """
                return await conn.fetchval(sql, pattern) or 0
        except Exception as e:
    ...
    @staticmethod
    async def get_countries_with_airports_count_by_mode(query: str, mode: str, lang: str = 'en') -> int:
        ...
        try:
            ...
            async with db.get_connection() as conn:
                ...
                sql = f"""
                    ...
                    FROM airports a
                    WHERE LOWER(COALESCE(a.name_translations->>'{lang}', a.name)) LIKE LOWER($1)
                """
                return await conn.fetchval(sql, pattern) or 0
        except Exception as e:
    ...
    @staticmethod
    async def get_countries_count_by_prefix(query: str, lang: str = 'en') -> int:
    ...
    @staticmethod
    async def get_cities_count_by_prefix(query: str, lang: str = 'en') -> int:
    ...
    @staticmethod
    async def get_airports_count_by_prefix(query: str, lang: str = 'en') -> int:
    ...
    @staticmethod
    async def get_airport_by_code(airport_code: str, lang: str = 'en') -> Optional[Dict[str, Any]]:
    ...
    @staticmethod
    async def get_city_by_code(city_code: str, lang: str = 'en') -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with db.get_connection() as conn:
                sql = f"""
                    SELECT
                        c.code,
                        COALESCE(c.name_translations->>'{lang}', c.name) AS name,
                        c.country_code,
                        c.time_zone,
                        c.coordinates,
                        co.name as country_name
    ...
    @staticmethod
    async def get_country_by_code(country_code: str, lang: str = 'en') -> Optional[Dict[str, Any]]:
            ...
            async with db.get_connection() as conn:
                ...
                sql = f"""
                    SELECT DISTINCT
                        ...
                        COALESCE(c.name_translations->>'{lang}', c.name) AS country_name
                    FROM countries c
                    JOIN cities city ON c.code = city.country_code
                    WHERE LOWER(COALESCE(city.name_translations->>'{lang}', city.name)) LIKE LOWER($1)
                ...
                for country_row in country_rows:
                    ...
                    sql_cities = f"""
                        ...
                        WHERE country_code = $1
                          AND LOWER(COALESCE(name_translations->>'{lang}', name)) LIKE LOWER($2)
                        ORDER BY name
                        LIMIT {settings.search_max_cities_per_country}
                    """
                ...
                for country_row in country_rows:
                    ...
                    sql_cities = f"""
                        SELECT DISTINCT
                            ...
                            COALESCE(city.name_translations->>'{lang}', city.name) AS city_name
                        FROM cities city
                        JOIN airports a ON city.code = a.city_code
                        WHERE a.country_code = $1
                          AND LOWER(COALESCE(a.name_translations->>'{lang}', a.name)) LIKE LOWER($2)
                        ORDER BY city_name
                        LIMIT {settings.search_max_cities_per_country_phase3}
                    """
                    ...
                    for city_row in city_rows:
                        ...
                        sql_airports = f"""
                            ...
                            WHERE city_code = $1
                              AND country_code = $2
                              AND LOWER(COALESCE(name_translations->>'{lang}', name)) LIKE LOWER($3)
                            ORDER BY name
                            LIMIT {settings.search_max_airports_per_city}
                        """
    ...
    async def get_country_center(country_code: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            async with db.get_connection() as conn:
                ...
                if total_airports == 0:
                    recommended_zoom = settings.country_zoom_levels.get("0", 5.0)
                elif total_airports < 5:
                    recommended_zoom = settings.country_zoom_levels.get("5", 5.0)
                elif total_airports < 15:
                    recommended_zoom = settings.country_zoom_levels.get("15", 4.5)
                elif total_airports < 40:
                    recommended_zoom = settings.country_zoom_levels.get("40", 3.5)
                elif total_airports < 100:
                    recommended_zoom = settings.country_zoom_levels.get("100", 2.6)
                else:
                    recommended_zoom = settings.country_zoom_levels.get("default", 1.8)
